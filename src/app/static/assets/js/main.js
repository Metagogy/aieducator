  ...
  //       },
		...
		googlemap: function() {
			...
			function init_gmap() {
				...
				var options = {
					center: {lat: 17.426983, lng: 78.419167},
					zoom: 15,
					mapTypeControl: true,
				...
				$('#googleMaps').gmap3({
					map: {
						options: options
                          ...
                          marker: {
                             latLng: [17.426983, 78.419167],
                             options: { icon: '/static/images/map-icon.png' },
                       }
                 });
			}
...
jQuery('#submit').on("click", function(e){ 
 ...
 $(document).ready(function() {
     ...
     //     $.cookie('pop', '1');
     // }
     $(".testCaseButton").click(event, function(){
       $.ajax({
          type: "POST",
          url: "/testcase/",
          data: {"id":$(this).val(), "value":$('input[name=choice'+$(this).val()+']:checked').val(), "p_id":$("#idd").val()},
          success: function(data) { 
            if (data.re == "True"){
              var found= $("#"+data.id)
              alert("Correct answer")
              found.closest("div").remove()             
            }
            else if(data.re == "False"){
              var found= $("#"+data.id)
              alert("Wrong answer")
              found.closest("div").remove()             
            }
